(function($){
	...
	$('[data-bg-image]').each(function(){$(this).css({ 'background-image': 'url('+$(this).data('bg-image')+')' }); });

	$('[data-bg-color]').each(function(){$(this).css({ 'background-color': $(this).data('bg-color') }); });

	$('[data-width]').each(function(){$(this).css({ 'width': $(this).data('width') }); });

	$('[data-height]').each(function(){$(this).css({ 'height': $(this).data('height') }); });

	$('[data-alpha]').each(function(){$(this).css({ 'opacity': $(this).data('alpha') }); });
	...
	$(document).ready(function(){
		...
		//     top: 100
		// });
		$('#hamburger-closed').on('click', function(){
			// Opening width 320px
			$('.vertical-menu').toggleClass("vertical-menu-open");
			$(this).toggleClass("close-button");
			$('.menu-social').toggleClass("uk-flex-column");
			$('.vertical-menu-wrapper-top').toggleClass("open");
			$('body').toggleClass("pushbody");
			$('.vertical-menu-close').toggleClass("rotatefull");
		});
		$("#hamburger-closed").one( "click", function() {
			$('.menu-social').removeClass('uk-flex-column');
		});
		...
		$('.card-testimonial-style').each(function(){
			var _swiper2 = $(this);
			_swiper2.swiper({
			    slidesPerView: 2,
			    pagination: '.swiper-pagination',
		...
		$('.tana-el-cart').each(function(){
			var _el = $(this),
				_args = _el.find('.el-query-data').text(),
				_atts = _el.find('.el-atts-data').text();
			...
			_el.find('.el-pagination a.loadmore').on('click', function(e){
				...
				$.post(theme_options.ajax_url, { action:'simplux_get_posts', page: _current, args: _args, atts: _atts }, function(response){
					try{
						var _json = $.parseJSON(response);
						if( _json.status == '1' ){
							_this.attr('data-current-page', _json.page);
							_el.find('.tana-card').append(_json.result);
						}
					}
					catch(e){}
				});
		...
		function collapse_overflow() {
			if( isTouchDevice() ) {
				$(document).on('touchstart click', function(e) {
				    ...
				    if (!$(target).is('.collapsed-item') && !$(target).parents().is('.collapsed-item')) {
				    	$('nav.main-nav > ul > li.menu-item-has-children > ul').hide();
				        $('nav.main-nav').removeClass('collapse-overflow');
				        // $(target).blur();
				    }
				});
				$('nav.main-nav > ul > li.menu-item-has-children').on('click', function(e){
					e.preventDefault();

					$('nav.main-nav').css({
						'left': -1*$('nav.main-nav').find('>ul').scrollLeft() + 'px',
						'margin-bottom': $('nav.main-nav').find('>ul').outerHeight() + 'px'
					});
					$('nav.main-nav').addClass('collapse-overflow');
					$(this).find('>ul').show();
		...
		$('.video-frame').on('click', function(event){
			...
			if( !$('body').hasClass('disable-video-lightbox') && _this.find('.video-player:not(.playpermalink)').length ){
	...
	$(window).load(function(){
		...
		$('.project-creative-carousel').each(function(){
				var swiperprojectcompletedcreative = new Swiper('.project-creative', {
		...
		$('.tana-card .uk-card-footer').each(function(){
...
/**
 * ANIMATED LOGO BEGINS HERE ANIMATED TEXT SVG ANIMATE
 */
...
(function() {
	...
	var _logo_color = jQuery('#logo-color').attr('data-lcolor');
	var options = [
		{ 	// word 1
			size : 80,
			weight : 5,
	...
	// items (li)
	items = [].slice.call(document.querySelectorAll('.site-branding'));

	function init() {
		
		items.forEach(function(item, pos) {
			var word = item.querySelector('.logo-text-link-animated'),
...
(function(){
	...
	var unit = 100;
	var _logo_color = jQuery('#color-ancient1').attr('data-lcolor');
	...
	function drawWave(fillcolor, alpha, zoom, delay) {
		...
		context.beginPath();
		drawSine(draw.t / 0.7, zoom, delay);
...
(function(){
	...
	var unit = 100;
	var _logo_color = jQuery('#color-ancient2').attr('data-lcolor');
	var canvas, context, canvas2, context2,height, width, xAxis, yAxis, draw;
